  ...
  if ($("#content").hasClass("fullpage")) {
    $(".fullpage").fullpage({
      ...
      afterLoad: function (origin, destination) {
        ...
        $(".subMenu > a").eq(currentIndex).addClass("on");
...
function resize() {
  ...
  if (winWidth >= 750) {
    ...
    $(".reset").css({
      transform: "translate(-50%, -50%) scale(" + contScale + ")",
    });
    $(".reset_top").css({
      transform: "translate(-50%, -0%) scale(" + contScale + ")",
    });
    $(".reset_bottom").css({
      transform: "translate(-50%, 0) scale(" + contScale + ")",
    });
