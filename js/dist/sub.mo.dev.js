...
$(document).ready(function () {
  // 디바이스 체크
  var isMobile = 'ontouchstart' in document.documentElement && /mobi/i.test(navigator.userAgent); //  페이지 로딩 후 세계관 텍스트 전체가 보인 후 에니메이션이 시작되는 문제 수정
  ...
  var readyCont = function readyCont() {
    ...
    $(".mainMenu a").each(function (i) {
      $(this).on("click", function () {
        var _this = $(this).parent(); // $('html, body').animate({scrollTop: $( ).offset().top}, 500);
